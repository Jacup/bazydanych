...
SELECT DISTINCT klient.nr, imie, nazwisko, opis, (data_wysylki-data_zlozenia) AS czas_realizacji, ilosc
   FROM ( ( klient INNER JOIN zamowienie 
                ON klient.nr = zamowienie.klient_nr
          ) INNER JOIN pozycja
            ON zamowienie.nr = pozycja.zamowienie_nr
          ...
          ON pozycja.towar_nr = towar.nr
   WHERE towar.opis LIKE 'chusteczki%' ORDER BY imie, nazwisko
;
